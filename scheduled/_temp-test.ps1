$PSScriptRoot = 'C:\Users\yasu\Documents\テスト会社\テストAIエージェント会社設立\これもテスト\scheduled'
# morning-standup.ps1
...
if (([DateTime]"2026-06-06").DayOfWeek -eq 'Sunday') {
    try {
...
if (-not (Test-Path $progressFile)) {
    $monthLabel = ([DateTime]"2026-06-06").Year.ToString() + "年" + ([DateTime]"2026-06-06").Month.ToString() + "月"
    $daysInMonth = [DateTime]::DaysInMonth(([DateTime]"2026-06-06").Year, ([DateTime]"2026-06-06").Month)

    # 現在フェーズを月番号から自動判定
    $monthNum = ([DateTime]"2026-06-06").Month
...
if (Test-Path $dailyFile) {
    ...
    $saturdayPrefix = ""
    if (([DateTime]"2026-06-06").DayOfWeek -eq 'Saturday') {
        $yearMonth = "2026-06"
