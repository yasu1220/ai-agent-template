...
if (([DateTime]"2026-06-10").DayOfWeek -eq 'Sunday') {
    try {
...
if (-not (Test-Path $progressFile)) {
    $monthLabel = ([DateTime]"2026-06-10").Year.ToString() + "年" + ([DateTime]"2026-06-10").Month.ToString() + "月"
    $daysInMonth = [DateTime]::DaysInMonth(([DateTime]"2026-06-10").Year, ([DateTime]"2026-06-10").Month)

    # 現在フェーズを月番号から自動判定
    $monthNum = ([DateTime]"2026-06-10").Month
...
if (Test-Path $dailyFile) {
    ...
    $saturdayPrefix = ""
    if (([DateTime]"2026-06-10").DayOfWeek -eq 'Saturday') {
        $yearMonth = "2026-06"
