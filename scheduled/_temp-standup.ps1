...
if (([DateTime]"2026-06-11").DayOfWeek -eq 'Sunday') {
    try {
...
if (-not (Test-Path $progressFile)) {
    $monthLabel = ([DateTime]"2026-06-11").Year.ToString() + "年" + ([DateTime]"2026-06-11").Month.ToString() + "月"
    $daysInMonth = [DateTime]::DaysInMonth(([DateTime]"2026-06-11").Year, ([DateTime]"2026-06-11").Month)

    # 現在フェーズを月番号から自動判定
    $monthNum = ([DateTime]"2026-06-11").Month
...
if (Test-Path $dailyFile) {
    ...
    $saturdayPrefix = ""
    if (([DateTime]"2026-06-11").DayOfWeek -eq 'Saturday') {
        $yearMonth = "2026-06"
